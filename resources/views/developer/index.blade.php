...
@section('content')
    <div class="max-w-7xl mx-auto mt-8">
        ...
        <x-shared.table title="API Tokens">
            <x-slot:actions>
                <form method="POST" action="{{ route('developer.createToken') }}" class="flex items-center gap-3">
                    ...
                    <button type="submit"
                        ...
                        <i class="fa-solid fa-plus mr-1"></i> Generate Token
                    </button>
                </form>
            </x-slot:actions>
            ...
            <table class="w-full text-left table-auto">
                <thead>
                <tr>
                    <th class="pl-10 xl:pl-24 h-20 bg-white text-left text-sm font-semibold uppercase text-gray-500">Token ID</th>
                    <th class="text-left h-20 bg-white text-sm font-semibold uppercase text-gray-500">Created</th>
                    <th class="text-left h-20 bg-white text-sm font-semibold uppercase text-gray-500">Token Name</th>
                    <th class="text-left h-20 bg-white text-sm font-semibold uppercase text-gray-500">Token</th>
                    ...
                    <tr class="hover:bg-gray-50 border-u border-gray-100 transition">
                        ...
                        <td class="px-6 py-4 text-right">
                            <a data-modal-target="delete-token-{{ $token->id }}" data-modal-toggle="delete-token-{{ $token->id }}"
                               class="text-red-600 hover:text-red-800 transition cursor-pointer" title="Delete">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-6 text-center text-gray-500">No tokens found.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
