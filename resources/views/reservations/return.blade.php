...
@section('content')
    <div class="max-w-2xl mx-auto bg-white p-8 rounded-lg shadow">
        <h2 class="text-2xl font-semibold mb-6">{{ __('return_product') }}: {{ $reservation->product->name }}</h2>

        <form method="POST" action="{{ route('reservations.return.submit', $reservation) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('upload_photo_product') }}</label>
