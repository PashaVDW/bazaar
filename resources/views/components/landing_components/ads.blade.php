<div class="bg-white py-10 px-6 rounded-lg shadow-md">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">{{ __('messages.our_offers') }}</h2>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        @foreach($ads->take(4) as $ad)
            <div class="bg-gray-100 p-4 rounded-lg">
                ...
                <p class="text-orange-600 font-semibold mt-2">€{{ number_format($ad->hourly_price, 2) }} / {{ __('messages.hour') }}</p>
            </div>
        @endforeach
    </div>
</div>
