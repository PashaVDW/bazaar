        ...
        <div class="container px-4 mx-auto">
            @if($business && $business->contract_file_path)
                ...
                <div class="w-full bg-white rounded-3xl shadow px-6 py-8">
                    <h2 class="text-xl font-semibold text-gray-800 mb-4">{{ __('messages.upload_signed_contract') }}</h2>
                    <p class="text-gray-600 mb-4">
                        {{ __('messages.contract_info') }}
                    </p>
                    <form action="{{ route('profile.contract.upload.save') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <label for="contract_file"
                            ...
                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                ...
                                <p class="mb-2 text-sm text-gray-500"><span class="font-semibold">{{ __('messages.click_to_upload') }}</span> or drag a file here</p>
                                <p class="text-xs text-gray-500">{{ __('messages.pdf_only') }}</p>
                        ...
                        <div class="text-right mt-6">
                            <button type="submit"
                                ...
                                <i class="fa-solid fa-paper-plane mr-2"></i> {{ __('messages.upload_contract') }}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="w-full bg-white rounded-3xl shadow p-10 text-center">
                    <div class="flex flex-col items-center justify-center">
                        ...
                        <h2 class="text-2xl font-semibold text-gray-800 mb-2">
                            {{ __('messages.no_signed_contract') }}
                        </h2>
                        <p class="text-gray-600 text-lg max-w-xl">
                            {{ __('messages.no_signed_contract_info') }}
                        </p>
                    </div>
                </div>
            @endif
