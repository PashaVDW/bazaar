...
@section('content')
    <div class="max-w-4xl mx-auto mt-12">
        <h1 class="text-3xl font-bold text-gray-800 mb-6">{{ __('customize_look_and_feel') }}</h1>
        ...
        <form id="settings-form" action="{{ route('profile.settings.update') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('company_logo') }}</label>
                ...
                <label id="logo-dropzone"
                    ...
                    {{ __('drop_or_click_to_upload_logo') }}
                </span>
                ...
                <p id="logo-filename" class="text-sm text-gray-600 mt-2 text-center hidden">
                    {{ __('selected_logo') }}: <span class="font-medium text-gray-800"></span>
                </p>
            </div>


            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('primary_color') }}</label>
                @php
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-4">{{ __('font') }}</label>
                @php
                    $fonts = [
            ...
            <div id="preview" class="rounded-xl border border-gray-200 p-6 bg-gray-50 shadow-inner transition-all">
                <h2 class="text-xl font-bold mb-2">{{ __('live_preview') }}</h2>
                <div class="h-2 w-32 rounded-full mb-4" id="preview-color" style="background-color: {{ $selected }}"></div>
                <p id="preview-text" class="text-gray-700" style="font-family: '{{ $selectedFont }}', sans-serif">{{ __('your_style_preview') }}</p>
            </div>
            ...
            <div class="pt-2">
                <button type="submit"
                    ...
                    <i class="fa-solid fa-floppy-disk mr-2"></i> {{ __('save_settings') }}
                </button>
            </div>
        </form>
    </div>
